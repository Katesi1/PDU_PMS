...
<div class="container-fluid mt-4">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <div>
            <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-chart-bar me-2"></i> Báo cáo thống kê</h1>
            <p class="text-muted">Thống kê và báo cáo về việc sử dụng phòng, đặt phòng và hoạt động người dùng</p>
        </div>
        <div>
                                ...
                                <?php if (isset($data['bookings']) && is_array($data['bookings'])): ?>
                                    <?php foreach ($data['bookings'] as $booking): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge bg-<?= 
                                                    ($booking['status'] ?? '') == 'đã xác nhận' ? 'success' : 
                                                    (($booking['status'] ?? '') == 'chờ xác nhận' ? 'warning' : 
                                                    (($booking['status'] ?? '') == 'đã hủy' ? 'danger' : 'secondary')) 
                                                ?>">
                                ...
                                <?php if (isset($data['maintenance']) && is_array($data['maintenance'])): ?>
                                    <?php foreach ($data['maintenance'] as $item): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge bg-<?= 
                                                    ($item['status'] ?? '') == 'đã hoàn thành' ? 'success' : 
                                                    (($item['status'] ?? '') == 'đang xử lý' ? 'warning' : 
                                                    (($item['status'] ?? '') == 'chưa xử lý' ? 'danger' : 'secondary')) 
                                                ?>">
