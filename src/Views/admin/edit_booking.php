...
<div class="p-6 bg-gray-50">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <form method="POST" action="/pdu_pms_project/public/admin/edit_booking?id=<?php echo htmlspecialchars($data['booking']['id']); ?>" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div id="teacher_select" style="display: <?php echo (isset($_POST['user_type']) ? ($_POST['user_type'] === 'student' ? 'none' : 'block') : ($data['booking']['teacher_id'] ? 'block' : 'none')); ?>;">
                    <label for="teacher_id" class="block text-sm font-medium text-gray-700 mb-1">Chọn giảng viên</label>
                    <select name="teacher_id" id="teacher_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" <?php echo (isset($_POST['user_type']) ? ($_POST['user_type'] === 'teacher' ? 'required' : '') : ($data['booking']['teacher_id'] ? 'required' : '')); ?>>
                ...
                <div id="student_select" style="display: <?php echo (isset($_POST['user_type']) ? ($_POST['user_type'] === 'student' ? 'block' : 'none') : ($data['booking']['student_id'] ? 'block' : 'none')); ?>;">
                    <label for="student_id" class="block text-sm font-medium text-gray-700 mb-1">Chọn sinh viên</label>
                    <select name="student_id" id="student_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" <?php echo (isset($_POST['user_type']) ? ($_POST['user_type'] === 'student' ? 'required' : '') : ($data['booking']['student_id'] ? 'required' : '')); ?>>
                ...
                <div>
                    ...
                    <input type="text" name="class_code" id="class_code" value="<?php echo htmlspecialchars($_POST['class_code'] ?? $data['booking']['class_code']); ?>" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>

                <div>
                    <label for="start_time" class="block text-sm font-medium text-gray-700 mb-1">Thời gian bắt đầu</label>
                    <input type="datetime-local" name="start_time" id="start_time" value="<?php echo htmlspecialchars($_POST['start_time'] ?? date('Y-m-d\TH:i', strtotime($data['booking']['start_time']))); ?>" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>

                <div>
                    <label for="end_time" class="block text-sm font-medium text-gray-700 mb-1">Thời gian kết thúc</label>
                    <input type="datetime-local" name="end_time" id="end_time" value="<?php echo htmlspecialchars($_POST['end_time'] ?? date('Y-m-d\TH:i', strtotime($data['booking']['end_time']))); ?>" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>
            ...
            <div class="mt-8 mb-4">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    ...
                    foreach ($data['rooms'] as $room):
                        ...
                        $roomClass = $isCurrentRoom ? 'bg-yellow-100 border-yellow-300' : ($isRoomAvailable ? 'bg-green-100 border-green-300 hover:bg-green-200 cursor-pointer' : 'bg-red-100 border-red-300 opacity-50 cursor-not-allowed');
                    ?>
                        <label class="relative">
                            <input type="radio" name="room_id" value="<?php echo htmlspecialchars($room['id']); ?>"
                                ...
                                class="hidden">
                            <div class="room-card border rounded-md p-3 text-center <?php echo $roomClass; ?> <?php echo (isset($_POST['room_id']) ? ($_POST['room_id'] == $room['id'] ? 'room-selected' : '') : ($isCurrentRoom ? 'room-selected' : '')); ?>">
            ...
            <div class="mt-8 flex justify-end">
                ...
                <button type="submit" class="bg-indigo-600 py-2 px-4 rounded-md text-white hover:bg-indigo-700 transition duration-300">Cập nhật đặt phòng</button>
            </div>
        </form>
    </div>
</div>
        ...
        document.querySelectorAll('input[name="room_id"]').forEach(function(input) {
            ...
            if (input.checked) {
                ...
                if (statusBadge) {
                    ...
                    statusBadge.classList.remove('bg-yellow-200', 'text-yellow-800', 'bg-green-200', 'text-green-800', 'bg-red-200', 'text-red-800');
