...
class AdminController
{
    ...
    public function editUser($data)
    {
        ...
        if (isset($data['edit_user'])) {
            $full_name = $data['full_name'] ?? $data['username'];
            $this->userModel->updateUser($data['id'], $data['username'], $data['email'], $data['role'], $data['class_code'] ?? null, $full_name);
            header('Location: /pdu_pms_project/public/admin/manage_users');
            exit;
        }
    ...
    // Quản lý loại phòng
    public function manageRoomTypes()
    {
        if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'admin') {
            header('Location: /pdu_pms_project/public/login');
            exit;
